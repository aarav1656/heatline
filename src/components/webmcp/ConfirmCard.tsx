    ...
    >
      <div className="w-full max-w-xl border border-hair-strong bg-paper text-ink">
        ...
        <div className="px-4 py-4">
          <h2 className="plate text-[1.375rem] leading-tight">{pending.title}</h2>
          <p className="mt-1 text-[1rem]">{pending.summary}</p>

          {pending.details && pending.details.length > 0 && (
            <dl className="mt-3 grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 border-t border-hair pt-3 text-[0.8125rem]">
              {pending.details.map((d) => (
                <div key={d.label} className="contents">
                  <dt className="colhead">{d.label}</dt>
                  <dd>{d.value}</dd>
                </div>
              ))}
            </dl>
