...
function redisRest(name: BackendName, url: string, token: string): StoreBackend {
  ...
  const key = (id: string, v: number) => `otc:case:${id}:v${pad(v)}`;
  const head = (id: string) => `otc:case:${id}:head`;
  ...
  return {
    ...
    async listIds() {
      const keys = (await cmd<string[]>(["KEYS", "otc:case:*:head"])) ?? [];
      return keys.map((k) => k.slice("otc:case:".length, -":head".length));
    },
  };
}
